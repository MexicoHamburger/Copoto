...
try:
    df = pd.read_csv('hate_speech_dataset2.csv')
    print("데이터셋을 성공적으로 불러왔습니다.")
    print(f"전체 데이터셋 크기: {len(df)}")
except FileNotFoundError:
    print("오류: 'combined_hate_speech_dataset.csv' 파일을 찾을 수 없습니다.")
    exit()
...
# (수정) 평가 데이터셋을 별도의 CSV 파일로 저장
test_csv_path = 'test_kmhas_dataset.csv'
test_df.to_csv(test_csv_path, index=False, encoding='utf-8-sig')
print(f"\n평가용 데이터셋을 '{test_csv_path}' 파일로 저장했습니다.")
print(f"학습 데이터: {len(train_df)}건, 검증 데이터: {len(val_df)}건, 평가 데이터: {len(test_df)}건")
...
training_args = TrainingArguments(
    output_dir="./kcelectra_kmhas_hate_speech_lora",
    learning_rate=2e-5,
...
# 8. 학습된 모델 저장
trainer.save_model("kmhas_kcelectra_lora_model")
print("\n모델 학습이 완료되었으며 'kmhas_kcelectra_lora_model' 디렉토리에 저장되었습니다.")